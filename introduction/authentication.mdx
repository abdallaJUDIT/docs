---
title: "Autenticação na Judit API"
sidebarTitle: "Autenticação"
description: "A Judit API usa autenticação via header HTTP customizado api-key (não Bearer). Aprenda a configurar a chave em diferentes linguagens, gerenciar variáveis de ambiente e proteger suas credenciais."
keywords: ["api key", "autenticação", "authentication", "header", "api-key", "credenciais", "bearer", "oauth"]
icon: "key"
---
...
1. **Entre em contato** com nossa equipe [comercial via WhatsApp](https://api.whatsapp.com/send/?phone=5511936186732).
...
Abaixo, mostramos como realizar uma requisição autenticada utilizando a URL Base de Consultas Assíncronas (https://requests.production.judit.io), consumindo a chave a partir de variáveis de ambiente.
