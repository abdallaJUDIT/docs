...
* **Consulta Síncrona (Datalake Hotstorage):** A resposta com todos os processos é devolvida instantaneamente no corpo (`body`) do próprio `POST`. Não exige checagem de status. Ideal para velocidade extrema. *[Veja a documentação Síncrona aqui](/cache-judit/hotstorage).*
...
Você pode filtrar processos por códigos numéricos específicos da Tabela do CNJ. A lógica é muito parecida com a dos tribunais.

* **Assuntos (`subject_codes`):** Usa as listas `contains` (Quero processos que tenham estes códigos) e `not_contains` (Não quero processos que tenham estes códigos).
* **Classes (`classification_codes`):** Usa a lista `keys` e a regra `not_equal` (igual aos tribunais).

<Accordion title="Exemplo JSON: Assuntos e Classes">
```json
{
    "search": {
        "search_type": "cpf",
