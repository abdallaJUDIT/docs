...
| `search_params.should_search_superior_instances` | boolean | Não | Se `true`, a Judit também busca o processo em **instâncias superiores** (3ª instância / tribunais superiores), além da 1ª e 2ª instância já cobertas por padrão. |

<Warning>
**Instâncias superiores têm custo adicional de 30%:** Ativar `search_params.should_search_superior_instances: true` faz a Judit também buscar o processo em instâncias superiores (3ª instância / tribunais superiores). Por ser uma captura mais custosa, essa requisição é cobrada com **+30% sobre o valor padrão** de uma Busca Processual Assíncrona.
</Warning>
