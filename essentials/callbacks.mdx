...
Quando uma busca ou um monitoramento e feito pela API da JUDIT enviamos um retorno com os processos encontrados.
...
Exemplo de um payload tipo de resposta, onde o campo `response_data` pode variar de acordo com os parâmetros da busca.
...
Se voce realizar uma busca com o response_type campo passando como valor 'lawsuit', o campo `response_data` vai retornar um objeto com o formato parecido com o seguinte;
...
Uma busca com o `response_type` campo passando como valor 'parties', 'attachments' ou 'lawsuits', o campo `response_data` vai retornar a resposta de forma paginada, onde `page_data` será um array que também pode variar de acordo com o `response_type` informado:
...
Se voce realizar uma busca com o `response_type` campo passando como valor 'parties', o campo `page_data` vai retornar um array com o formato parecido com o seguinte:
...
Se voce realizar uma busca com o `search_params` objeto passando como parâmetro o campo `masked_response` sendo true, o campo `response_data` ou `page_data` se for paginado, vai retornar um objeto com as informações minificadas com o formato parecido com o seguinte:
...
Caso o processo consultado de tenha um ou mais processos relacionado ou em alguma outra instância, esses processos serão retornados em um array `related_lawsuits.