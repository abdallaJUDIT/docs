...
<Steps>
  <Step title="Criação do monitoramento" stepNumber="1">
    ...
    - `search_key`: O número do documento(CPF, CNPJ, OAB ou name) que você deseja buscar;
  ...
  <Step title="Atualizando um Monitoramento de novas ações" stepNumber="4">
    ...
    Esta rota permite atualizar o tracking com campos opcionais, como `recurrence` e o objeto `search`. No objeto `search`, podem ser fornecidos os campos `search_key` e os filtros definidos em `search_params`.

    Aqui está um exemplo de como fazer isso usando curl:

    ```bash
    curl --request PATCH \
      --url https://tracking.prod.judit.io/tracking/412cf8d8-c4bc-4fe2-b6e3-570000000001 \
      --header 'Content-Type: application/json' \
      --header 'api-key: <YOUR-API-KEY>' \
      --data '{
      "search": {
        "search_type": "cnpj",
        "search_key": "99.999.999/0009",
        "search_params": {
          "filter": {
            "side": "Active",
