...
A JUDIT permite que você monitore processos, CPFs, CNPJs, OAB ou número do processo, mantendo-o atualizado sobre qualquer nova movimentação.

O monitoramento é a automatização das consultas processuais do item anterior.

Diariamente, nossa API busca em todos os tribunais do Brasil processos relacionados ao CPF, CNPJ, OAB ou número de processo que você está monitorando.

## Rota de Monitoramento

Para começar a monitorar um processo, CPF, CNPJ, OAB ou processo, você deve realizar uma solicitação POST para a rota `/tracking`.
...
`status`: retorna monitoramentos ao status ou grupo de status 'created', 'updating', 'updated', 'paused', 'deleted';
...
## Retomar um Monitoramento

Para retomar um monitoramento pausado, você pode fazer uma solicitação POST para a rota `/tracking/{monitoramento}/resume`, substituindo `{monitoramento}` pelo ID do monitoramento que você deseja pausar.
