...
| :--- | :--- | :--- | :--- |
| `search.search_type` | string | **Yes** | Entity type: `"cpf"`, `"cnpj"`, `"oab"`, `"name"` or `"lawsuit_cnj"`. |
| `search.search_key` | string | **Yes** | Value to search (e.g. `"999.999.999-99"`, `"John Smith"`). |
| `process_status` | boolean | No | If `true`, the response includes `phase` and `status` fields per lawsuit. |
...
| `response_data[].phase` | string | (Optional, with `process_status: true`) Current lawsuit phase — see table below. |
| `response_data[].status` | string | (Optional, with `process_status: true`) Current lawsuit status — see table below. |
...
<Accordion title="See response example with process_status: true">
