...
| HTTP | Typical `error.message` | When it happens | How to resolve |
| :--- | :--- | :--- | :--- |
| `500` | `INTERNAL_SERVER_ERROR` | Unexpected Judit-side failure. | Retry with backoff. If it persists, open a ticket at `atendimento@judit.io` referencing the `request_id`. |
...
| HTTP 403 on a specific route | Feature not enabled in your plan. | Talk to Judit at `atendimento@judit.io`. |
