...
<Note>
  The `reference_type` field can be either `request` or `tracking`.
</Note>

### Response sent for tracking

Based on the recurrence configured for the tracking, the crawler is executed to:

- Perform the lawsuit search at the court, in the case of lawsuit tracking.

- Search for new cases linked to the document, in the case of new-cases tracking.

When the crawler finds new information, a message with the updated content is sent via webhook to:

- The webhook registered on the account, or

- The webhook provided as a parameter in the request payload.

If there are no updates to the lawsuit or no new cases at the time of crawler execution, only the request_completed message is sent.

<Note> When the tracking sends its response to the webhook, the `origin_id` field will have the same value as `tracking_id`, which is the unique identifier of the tracking. </Note>

### Request_completed

Below is an example of the response sent at the end of a full delivery, with the confirmation message indicating that all responses have been delivered. This final message includes the statusCode <strong>"600"</strong> and the message <strong>"request_completed"</strong>, as illustrated below:
...
Occasionally, an `application_error` message may be sent if the lawsuit or a specific instance is not found.
```json
{
  "user_id": "7f8065a3-4891-428d-9456-dedfc12ff850",
