...
keywords: ["new lawsuits", "tracking by document", "cpf", "cnpj", "oab", "new lawsuits", "watcher"]
icon: "satellite-dish"
---
...
var STEPS=[
  ...
  {fr:'judit',to:'court',label:'checks for new lawsuits',note:'for the monitored CPF/CNPJ',col:T,ts:4.8,te:5.6,yp:0.60},
  {fr:'court',to:'judit',label:'new lawsuit filed',note:'',col:J,ts:6.1,te:6.9,yp:0.72},
...
function draw(ctx,w,h,t){
  ...
  var nodeY=h*0.21;
  ctx.fillStyle=J;ctx.font='bold 12px Inter,system-ui,sans-serif';ctx.textAlign='center';ctx.textBaseline='middle';ctx.fillText('Document Tracking',w/2,h*0.04);
  ctx.fillStyle=M;ctx.font='10px Inter,system-ui,sans-serif';ctx.fillText('Judit detects new lawsuits linked to the monitored document',w/2,h*0.09);
...
To start monitoring a document, you must make a POST request to the `/tracking` route.

### Request Payload

The POST request must include a payload with the following properties:

- `search_type`: This field defines the type of entity to be searched. The possible values are: `cpf`, `cnpj`, `oab`, `name`, `lawsuit_cnj` or `lawsuit_id`. For lawsuit searches, we will specifically use `cpf`, `cnpj` or `oab`, which correspond to the document number.
...
- `search_params`: An object containing some search parameters such as:
  ...
  - `with_inferred_parties` (optional): If `true`, the tracking also considers lawsuits in which the link to the tracked document was inferred by Judit, instead of published by the court. Default: `false`. See [Party inference](#party-inference-search_params-with_inferred_parties);
  - `masked_response`: Defines whether the response will be returned minified. This parameter is only applicable to queries (simple or complete) by document in the lawsuit search context.
...
In the tracking responses, every party carries the `was_inferred` field: `false` when the document was published by the court itself and `true` when it was inferred by Judit (in that case the `documents` array usually comes empty and `main_document` is the result of the inference). Treat the absence of the field as `false`.

<Warning>
**Risk of homonyms.** Because the inference starts from the party's **name**, there is a chance of linking to the tracked document a lawsuit that actually belongs to another person or company with an identical or very similar name. The internal validations reduce this risk, but do not eliminate it. In searches with `search_type: "name"`, this risk adds up to the homonyms of the searched term itself.

In tracking the effect is continuous: a homonym's lawsuit that gets in through inference will keep generating notifications on every new case update. Use `was_inferred` to decide how to treat each lawsuit — an inferred link does not carry the same reliability as a document published by the court.
</Warning>
...
`search_key`: Returns trackings with the search related to the CPF, CNPJ, OAB or lawsuit number provided;
...
**Note**: Each new lawsuit distributed for the monitored document will be notified via webhook containing the full lawsuit data.
    ...
    "page_data": [
        ...
        },
        {
            ...
            "response_data": {
                ...
                "name": "TEST USER 1 X TEST USER 2",
                "parties": [
                    {
                        "side": "Active",
                        "name": "USER 1",
