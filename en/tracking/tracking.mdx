---
title: "Lawsuit Tracking with Webhooks"
sidebarTitle: "Tracking · Overview"
description: "Automatically track movements on lawsuits by CNJ, CPF, CNPJ or OAB. Judit polls courts at the configured recurrence and pushes updates via webhook."
keywords: ["tracking", "webhook", "recurrence", "update", "court", "watcher"]
icon: "radar"
---
...
**Judit Tracking** automates court watching: register a CNJ, CPF, CNPJ or OAB, define the recurrence (in days), and Judit polls the courts for you. Whenever something changes — new step, attachment or new lawsuit born — you get the update via **webhook** or e-mail.

> 🤖 Base endpoint: `POST https://tracking.production.judit.io/tracking`. Tracking runs in the background, within Judit's optimal concurrency window for each court. The first run happens within 24 hours after registration; subsequent runs follow the `recurrence` (in days) you set.
...
<CardGroup cols={2}>
  ...
  <Card title="Party watch" icon="user-magnifying-glass">
    Watch CPFs/CNPJs and get notified when **a new lawsuit** is filed against (or by) your client.
  </Card>
  <Card title="Continuous compliance" icon="shield-halved">
    On periodic KYC reviews, automatically detect new litigation involving the counterpart.
  </Card>
...
| Scenario | Dedicated page |
| --- | --- |
| Watch a **specific lawsuit** (CNJ) | this page |
| Detect **new lawsuits** linked to CPF/CNPJ/OAB | [By document](/en/tracking/tracking-document) |
| Advanced filters (court, amount, class, etc.) | [Custom](/en/tracking/tracking-custom) |
