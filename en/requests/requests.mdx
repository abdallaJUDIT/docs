...
| `search_params.should_search_superior_instances` | boolean | No | If `true`, Judit also searches the lawsuit at **superior instances** (3rd instance / higher courts), in addition to the 1st and 2nd instance already covered by default. |

<Warning>
**Superior instances have a 30% additional cost:** Enabling `search_params.should_search_superior_instances: true` makes Judit also search the lawsuit at superior instances (3rd instance / higher courts). Since this capture is more resource-intensive, the request is billed at **+30% over the standard price** of an Asynchronous Lawsuit Search.
</Warning>
