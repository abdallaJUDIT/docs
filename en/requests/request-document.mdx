...
| `search.search_params.with_inferred_parties` | boolean | No | If `true`, also includes lawsuits in which the link to the searched document was inferred by Judit. Default: `false`. |
...
Many courts do not publish the parties' CPF/CNPJ on the lawsuit cover page — only their names. In those cases a document search does not find the lawsuit, even when the person or company is in fact part of it.

By sending `with_inferred_parties: true` inside `search_params`, Judit also considers lawsuits in which the link to the searched document was **inferred** through internal validations (cross-checking the party name against other data already known in the base), instead of being read directly from the court. The practical effect is broader coverage of lawsuits linked to the document.

The parameter is opt-in: when omitted it defaults to `false`, and the search returns only the links explicitly published by the court.

<CodeGroup>
```json Query with party inference
{
    "search": {
        "search_type": "cpf",
        "search_key": "999.999.999-99",
...
#### How to identify what was inferred (`was_inferred`)

In the responses, each object of the `parties` array carries the `was_inferred` field, indicating where that party's identification came from:

| Value | Meaning |
| :--- | :--- |
| `false` | The party's document was published by the court itself. Identification confirmed at the source. |
| `true` | The party's document was inferred by Judit. In these cases the `documents` array usually comes empty and `main_document` is the result of the inference. |

<Note>
Not every party carries the `was_inferred` field. Treat its absence as `false` (identification not inferred).
</Note>

<Accordion title="See response example with an inferred party">
```json
{
...
Use `was_inferred` to decide how to treat each lawsuit: an inferred link does not carry the same reliability as a document published by the court.
</Warning>
