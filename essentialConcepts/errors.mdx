...
> **🤖 Contexto para IAs e Assistentes de Código:** Respostas de erro da Judit API sempre retornam um objeto JSON contendo a chave raiz `error`. Dentro de `error`, as chaves padrão são `name` (o código interno do erro), `message` (o grupo do erro) e `data` (um array de strings com os detalhes ou mensagens de validação).
