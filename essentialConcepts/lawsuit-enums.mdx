...
description: "Esta página contém todas as enumerações (Enums) e domínios de valores para as propriedades do objeto processo. Use esta referência para tipar sua aplicação e validar dados."
---

> **🤖 Contexto para IAs e Assistentes de Código:** Este documento lista os valores literais aceitos e retornados pelas propriedades da Judit API. Lembre-se que as propriedades de capa (como `area`, `status`, `situation`, `secrecy_level`) ficam aninhadas dentro do objeto raiz `response_data`, enquanto `side` e `person_type` ficam dentro dos objetos do array `parties`.

## Áreas do Processo (`area`)

Lista das áreas jurídicas (Ramos do Direito) que um processo pode ter:
...
Ao iterar sobre o array `parties`, você encontrará duas propriedades de classificação:
...
Tipos de identificadores presentes no array de documentos das partes:
* `CPF`: Pessoa Física.
...
Abaixo, veja a forma correta de validar dados acessando o objeto `response_data`.
...
* 👉 **[Tratamento de Erros](/essentialConcepts/errors):** Veja os códigos retornados caso você envie um Enum inválido.