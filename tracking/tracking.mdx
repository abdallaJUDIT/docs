---
title: "Monitoramento Processual com Webhooks"
sidebarTitle: "Monitoramento · Visão Geral"
description: "Acompanhe automaticamente movimentações em processos por CNJ, CPF, CNPJ ou OAB. A Judit consulta os tribunais com a recorrência configurada e envia atualizações via webhook."
keywords: ["monitoramento", "tracking", "webhook", "recorrência", "atualização", "tribunal", "watcher"]
icon: "radar"
---
...
O **Monitoramento JUDIT** automatiza a vigilância processual: você cadastra um CNJ, CPF, CNPJ ou OAB, define a recorrência (em dias) e a Judit consulta os tribunais para você. Sempre que houver novidade — nova movimentação, anexo ou processo nascido — você recebe a atualização via **webhook** ou e-mail.

> 🤖 Endpoint base: `POST https://tracking.production.judit.io/tracking`. O monitoramento roda em background, dentro da janela de melhor concorrência da Judit no tribunal. A primeira execução acontece nas 24 horas seguintes ao cadastro; as próximas seguem a recorrência definida em `recurrence` (em dias).
...
<CardGroup cols={2}>
  ...
  <Card title="Vigilância de partes" icon="user-magnifying-glass">
    Monitore CPFs/CNPJs e seja notificado quando **um novo processo** for ajuizado contra (ou por) seu cliente.
  </Card>
  <Card title="Compliance contínuo" icon="shield-halved">
    Em revisões periódicas de KYC, identifique automaticamente novos litígios envolvendo a contraparte.
  </Card>
...
| Cenário | Página dedicada |
| --- | --- |
| Monitorar **um processo específico** (CNJ) | esta página |
| Detectar **novos processos** vinculados a CPF/CNPJ/OAB | [Por documento](/tracking/tracking-document) |
| Filtros avançados (tribunal, valor, classe, etc.) | [Customizado](/tracking/tracking-custom) |
