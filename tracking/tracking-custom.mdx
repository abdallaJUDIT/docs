---
title: "Monitoramento Customizado por Filtros"
sidebarTitle: "Customizado"
description: "Configure monitoramentos com filtros avançados (tribunal, valor da causa, data de distribuição, partes) para acompanhar apenas o que importa na Judit API."
keywords: ["monitoramento customizado", "custom tracking", "filtros", "tribunal", "watcher avançado"]
icon: "searchengin"
---
...
var STEPS=[
  ...
  {fr:'judit',to:'court',label:'verifica novos processos',note:'com os filtros configurados',col:T,ts:4.8,te:5.6,yp:0.60},
  {fr:'court',to:'judit',label:'novo processo encontrado',note:'',col:J,ts:6.1,te:6.9,yp:0.72},
...
function draw(ctx,w,h,t){
  ...
  var nodeY=h*0.21;
  ctx.fillStyle=J;ctx.font='bold 12px Inter,system-ui,sans-serif';ctx.textAlign='center';ctx.textBaseline='middle';ctx.fillText('Monitoramento Customizado por Filtros',w/2,h*0.04);
  ctx.fillStyle=M;ctx.font='10px Inter,system-ui,sans-serif';ctx.fillText('Judit detecta novos processos que atendem aos filtros configurados',w/2,h*0.09);
...
Para começar a monitorar com filtros customizados, você deve realizar uma solicitação POST para a rota `/tracking`.
...
- `search_params`: Um objeto que contém alguns parâmetros da busca como:
  ...
  - `masked_response`: Define se a resposta virá minificada. Este parâmetro é aplicável apenas a consultas (simples ou completas) no contexto de busca processual.
...
<Note>
  Todos os monitoramentos customizados cadastrados são realizados ***on-demand***. Recomendamos verificar as condições de custo associadas a este serviço antes de sua utilização.
</Note>
...
<Note>
  Na resposta da criação do monitoramento, é retornado o campo `hour_range`, que indica o horário em que a consulta ao tribunal será realizada pela primeira vez.
</Note>
...
`search_type`: retorna monitoramentos do tipo de referência especificado **"custom"**, **"cpf"**, **"cnpj"**, **"oab"**, **"lawsuit_cnj"**, **name** ou **rji**;
...
**Obs**: Cada novo processo detectado pelos filtros do monitoramento customizado será notificado via webhook contendo o processo por completo.
...
```json
{
    ...
    "search": {
        ...
        "search_params": {
            "filter": {
                "distribution_date_gte": "2025-01-01T00:00:00.000Z"
            }
        }
    },
