...
* **Consulta Síncrona (Datalake Hotstorage):** A resposta com todos os processos é devolvida instantaneamente no corpo (`body`) do próprio `POST`. Não exige checagem de status. Ideal para fluxos sensíveis à latência da resposta como onboardings. *[Veja a documentação Síncrona aqui](/pt/cache-judit/hotstorage).*
...
👉 *[Aprenda a configurar e receber Webhooks aqui](/pt/webhook/callbacks)*
</Info>
