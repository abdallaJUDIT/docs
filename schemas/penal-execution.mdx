---
title: "Objeto Penal Execution (Execução Penal)"
description: "O objeto `penal_execution` representa um processo de Execução Penal (fase de cumprimento de pena). Ele detalha o andamento do cumprimento da sentença de um reeducando nos tribunais."
---

> 🤖 A estrutura do objeto de Execução Penal é semelhante à do [Objeto Lawsuit](/schemas/lawsuit), mas contém campos específicos como `free_justice` e um objeto dedicado para a `last_step` (última movimentação). Os papéis das partes (`person_type`) também diferem, assumindo valores como `EXECUTADO` ou `REEDUCANDO`.
...
* 👉 **[Consultas Penais](/requests/penal):** Veja a documentação do endpoint para iniciar uma extração em varas criminais e de execução.
