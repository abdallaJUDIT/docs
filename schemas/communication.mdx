...
description: "Estrutura do objeto Communication retornado pelo monitoramento do Diário de Justiça Eletrônico (DJE) na Judit API. Inclui dados da publicação, destinatários, advogados vinculados e texto integral."
keywords: ["communication", "diário oficial", "DJE", "intimação", "publicação", "schema", "monitoramento"]
icon: "newspaper"
---

> 🤖 O monitoramento do Diário de Justiça Eletrônico retorna o objeto `communication`. Cada objeto representa uma publicação do DJE em que houve correspondência com o termo monitorado. A raiz contém os metadados da publicação (datas, tribunal, tipo). Os destinatários ficam no array `recipients` e os advogados vinculados a eles em `recipient_lawyers`.
...
* **Destinatários (`recipients`):** Partes do processo diretamente citadas na publicação.
* **Advogados dos Destinatários (`recipient_lawyers`):** Advogados vinculados às partes, com dados de OAB.
* **Texto Integral (`text`):** Conteúdo completo da publicação conforme extraído do DJE.
...
| `date_availability` | string (ISO 8601) | Data em que a publicação ficou disponível no DJE para consulta. |
| `date_shipping` | string (ISO 8601) | Data de expedição/envio da comunicação pelo tribunal. |
| `hash` | string | Hash único da publicação, utilizado internamente para deduplicação. |
...
| `status` | string | Status da publicação no DJE (ex: `"P"` para publicado). |
| `text` | string | Texto integral da publicação conforme extraído do Diário de Justiça Eletrônico. |
...
| `middle` | string | Código abreviado do veículo de publicação (ex: `"D"` para DJE). |
...
| `communication_number` | number | Número sequencial da comunicação dentro da edição do DJE. |
...
Array com as partes do processo diretamente citadas na publicação. Todos os campos são obrigatórios.
...
* 👉 **[Monitoramento do Diário Oficial](/tracking/tracking-communication):** Veja como configurar o monitoramento do DJE e receber publicações via webhook.
