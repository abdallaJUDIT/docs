...
| Finalizado | O processo foi finalizado.    |
 ...
 <Step title="Consultando a quantidade de processos no datalake" stepNumber="3">
  ...
  É possivel consultar a quantidade de processos atrelados a um documento salvos em nosso datalake, para isso, basta realizar um requisição `POST` para o endpojnt `https://lawsuits.production.judit.io/requests/count` adicionando `search_type` e `search_key` no payload.
