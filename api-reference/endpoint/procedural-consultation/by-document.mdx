...
<Steps>
 <Step title="Criação de Request" stepNumber="1">
   Para realizar a consulta por documento, é necessário primeiramente criar uma requisição para o endpoint de consulta por documento.
   ...
   curl --request POST \
     ...
     A resposta da consulta será um objeto JSON com os dados do documento consultado:
     ...
     ```
    <Note>Na consulta por documento o search_type pode ser “oab”, “cpf” e “cnpj”.</Note>
 ...
 <Step title="Consultar o status da request" stepNumber="2">
     ```bash
      curl --request GET \
        --url https://requests.prod.judit.io/requests/{request-id-da-primeira-request}
        --header 'api_key: SUA-API-KEY'
        --header 'Content-Type: application/json'
     ```
 ...
 <Step title="Consultar o conteúdo da resposta"  stepNumber="3">
   <Steps>
    <Step title="">
        ```bash
        curl --request GET \
          --url https://requests.prod.judit.io/responses/?request_id={request-id-da-primeira-request}&page=1&page_size=10
          --header 'api_key: SUA-API-KEY'
          --header 'Content-Type: application/json'
        ```

     Retorno quando foi encontrado um processo no tribunal:
