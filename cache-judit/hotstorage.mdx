...
| :--- | :--- | :--- | :--- |
| `search.search_type` | string | **Sim** | Tipo da entidade buscada: `"cpf"`, `"cnpj"`, `"oab"`, `"name"` ou `"lawsuit_cnj"`. |
| `search.search_key` | string | **Sim** | O valor a ser buscado (ex.: `"999.999.999-99"`, `"João Silva"`). |
| `process_status` | boolean | Não | Se `true`, a resposta inclui os campos `phase` e `status` de cada processo. |
...
| `response_data[].phase` | string | (Opcional, com `process_status: true`) Fase atual do processo — ver tabela abaixo. |
| `response_data[].status` | string | (Opcional, com `process_status: true`) Status atual do processo — ver tabela abaixo. |
...
<Accordion title="Ver exemplo de resposta com process_status: true">
